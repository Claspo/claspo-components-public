...
export default class SysPromoCodeComponent extends WcElement {
  ...
  connectedCallback() {
    ...
    this.observeProps((prev, next) => {
      ...
      if (promoCodeValue) {
        inlineEditElement.innerText = promoCodeValue;
      } else {
      ...
      if (lastPromoCodeRecord) {
        this._handleContextRecord(lastPromoCodeRecord);
      }
  ...
  _handleContextRecord(record) {
    ...
    if (record.value.viewIndex >= model.path[0]) {
      return;
    }

    const lastPromoCode = this.services.context.getRecord(LAST_PROMOCODE_RECORD_KEY)?.value.value || null;
    ...
    if (!!lastPromoCode) {
      ...
      textInnerElement.innerText = lastPromoCode;
    } else {
